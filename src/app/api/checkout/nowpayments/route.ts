...
import { createNowPaymentsInvoice } from '@/utils/nowpayments';
...
export async function POST(req: Request) {
  try {
    ...
    const sessionId = `nowpayments_${Date.now()}_${Math.random().toString(36).substring(2, 8)}`;
    ...
    if (insertErr) {
      console.error('[NOWPayments Checkout] Error creating orders:', insertErr);
      return NextResponse.json({ error: 'Failed to create order records' }, { status: 500 });
    }

    // Call NOWPayments Invoice API
    const invoice = await createNowPaymentsInvoice({
      amount: Number(finalTotalUsd.toFixed(2)),
      currency: currency.toLowerCase(),
      orderId: sessionId,
      orderDescription: `UpStore Digital Order (${items.length} items)`,
    });
    ...
    return NextResponse.json(
      { error: error.message || 'Failed to initiate NOWPayments checkout' },
      { status: 500 }
    );
  }
}
